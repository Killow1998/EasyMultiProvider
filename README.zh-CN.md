...
- 在“性能与健康”中查看近期模型的 TTFT/TPS 中位数；实际采集到 Fast 请求时，
  再区分 OpenAI 速度模式。同时显示成功、429、502、503 和 504 的观测比例；
  不保存提示词或回复内容。
...
EMP 支持 Codex CLI `0.149.x` 至 `0.152.x`，推荐使用 `0.152.x`。Web UI 会显示
...
EMP v0.9.0 至 v0.9.7 使用同一种加密迁移格式。当前版本可以导入其中任意版本
