...
| Distribution | The current Linux release package assembled and passed packaged executable/service smoke, user-install checks, actual isolated self-update and failed-startup rollback. The builder works with directly installed Cargo 1.93.1 and retains rustup selection where available. The final rebuilt release served Web, configuration, catalog and real subscription traffic on port 4201. Package CI `36095686559` passed Linux x64, Windows x64, macOS Intel and Apple Silicon at `ed1509f` with the updated Python pin. The memory change is not yet in those cross-platform packages | Verify the current release on all platforms, then perform reversible formal cutover |
